...
<script>
    ...
    video.addEventListener('play', async () => {
        ...
        setInterval(async () => {
            if (processingFace) return; // Jangan proses jika sedang dalam konfirmasi atau proses lain
            ...
            if (resized.length > 0) {
                const bestMatch = faceMatcher.findBestMatch(resized[0].descriptor); // Asumsi hanya satu wajah terbesar
                ...
                if (bestMatch.label !== "unknown") {
                    processingFace = true; // Set flag untuk mencegah deteksi berulang
                    statusText.innerText = `🔍 Wajah terdeteksi: ${bestMatch.label}`;
                    statusText.className = "text-center mt-3 text-info fw-bold";

                    // Kirim data ke backend untuk validasi jadwal dan aksi
                    try {
                        ...
                        if (result.status) {
                            ...
                            Swal.fire({
                                ...
                                if (confirmResult.isConfirmed) {
                                    // Kirim permintaan absensi ke endpoint sesuai aksi
                                    ...
                                    if (absensiResult.status) {
                                        ...
                                        statusText.className = "text-center mt-3 text-danger fw-bold";
                                        processingFace = false; // Reset flag untuk coba lagi
                                    }
                                } else {
                            ...
                            // Pesan error dari backend jika tidak ada jadwal cocok atau sudah absen
                            Swal.fire({
                                toast: true,
                                position: 'top-end',
                                icon: 'warning',
                    ...
                    statusText.className = "text-center mt-3 text-warning fw-bold";
                    processingFace = false; // Reset flag jika wajah tidak dikenali
                }
            } else {
                statusText.innerText = "🔎 Mencari wajah...";
                statusText.className = "text-center mt-3 text-primary fw-bold";
                processingFace = false; // Reset flag jika tidak ada wajah terdeteksi
            }
        }, 1000); // Interval deteksi wajah
    });
    async function loadLabeledImages() {
        const users = {
            !!json_encode(\App\ Models\ User::whereNotNull('image') - > get(['nama', 'image'])) !!
        };
